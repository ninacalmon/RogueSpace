...
func change_scene_to(
	scene_path: String, time_to_fade_in: float = 0.2, time_to_fade_out: float = 0.4
) -> void:
	...
	MusicManager.changing_scene(scene_path)

	color_rect_tween = create_tween().set_trans(Tween.TRANS_SINE)
	color_rect_tween.tween_property(
	color_rect, "modulate:a", 1.0, time_to_fade_in
).finished.connect(_load_new_scene.bind(scene_path))
