...
func _process(_delta):
	...
	var mothership_screen = Vector2(viewport_size.x / 2.0, viewport_size.y / 2.0) + (mothership.global_position - camera_global_pos)
	...
	if not is_visible_on_screen:
		...
		if just_left_screen:
			...
			rotation = lerp_angle(rotation, target_rotation, clamp(smooth_speed * get_process_delta_time(), 0.0, 1.0))
