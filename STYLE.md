...
* Use **UTF-8** encoding without a [byte order mark](https://en.wikipedia.org/wiki/Byte%5Forder%5Fmark). _(editor default)_
...
This section focuses on code order. For formatting, see [Formatting](#formatting). For naming conventions, see [Naming conventions](#naming-conventions).
...
Follow with the optional `@icon` then the `class_name` if necessary. You can turn a GDScript file into a global type in your project using `class_name`. For more information, see [Registering named classes](https://docs.godotengine.org/en/stable/tutorials/scripting/gdscript/gdscript_basics.html#doc-gdscript-basics-class-name). If the class is meant to be an [abstract class](https://docs.godotengine.org/en/stable/tutorials/scripting/gdscript/gdscript_basics.html#doc-gdscript-basics-abstract-class), add `@abstract` _before_ the `class_name` keyword.

Then, add the `extends` keyword if the class extends a built-in type.

Following that, you should have the class's optional [documentation comments](https://docs.godotengine.org/en/stable/tutorials/scripting/gdscript/gdscript_documentation_comments.html#doc-gdscript-documentation-comments). You can use that to explain the role of your class to your teammates, how it works, and how other developers should use it, for example.
...
GDScript supports [optional static typing](https://docs.godotengine.org/en/stable/tutorials/scripting/gdscript/static_typing.html#doc-gdscript-static-typing).
...
This option is considered more [type-safe](https://docs.godotengine.org/en/stable/tutorials/scripting/gdscript/static_typing.html#doc-gdscript-static-typing-safe-lines) than type hints, but also less null-safe as it silently casts the variable to `null` in case of a type mismatch at runtime, without an error/warning.

---

## Additional Rules for Code Modifications

### Variable Re-assertion Rule

When modifying any GDScript code, if a variable is reassigned or rewritten during the modification process, it is CRUCIAL to re-assert its original value immediately after the modification. This rule applies especially to `@export` variables, which are editor-exposed properties that may be modified by the Godot editor or user input.

**Rationale:** Exported variables represent contract boundaries between your script and the Godot editor. Changing their values without restoration can cause:
- Editor desynchronization (script value differs from Inspector display)
- Unintended runtime behavior when the game starts or scene loads
- Loss of user-configured values in the editor

**Required pattern:**
```gdscript
# Before modification - store original value
var original_value = my_exported_var

# During modification - safe to modify
my_exported_var = temporary_value

# AFTER modification - MUST restore original
my_exported_var = original_value
```

### Preservation of Functionality Rule

ALL code modifications must preserve exact original functionality. The ONLY allowed changes are:
1. **Ordering** - Reorganizing code blocks according to the code order guidelines above
2. **Formatting** - Applying style guide conventions (whitespace, indentation, line length)
3. **Cleaning** - Removing dead code, unused imports, or redundant comments

Forbidden changes include:
- Logic alterations (even "improvements")
- Behavior modifications (even bug "fixes" unless explicitly requested)
- Architecture changes without explicit instruction
- Feature additions or removals

**Verification criterion:** After any modification, the modified code must produce byte-for-byte identical runtime behavior compared to the original, when given identical inputs and scene state.

**LLM instruction:** When rewriting code, always maintain a mental diff check: "Does this change make the code do something different at runtime?" If yes, revert to original logic while keeping only formatting/ordering improvements.
